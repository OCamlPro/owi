 ...
 * the following condition: <code>exp_factor < (CC_MAX_ELEMENTS /
 * capacity)</code>.
 *
 ...
 */
enum cc_stat array_replace_at(Array *ar, void *element, size_t index,
                              void **out) {
  if (index >= ar->size)
    return CC_ERR_OUT_OF_RANGE;
 ...
 */
void array_remove_all(Array *ar) { ar->size = 0; }
 ...
 * allocation for the new subarray failed.
 */
 ...
 * original Array that return true on pred(element) without modifying the
 * original Array.
 *
 * @param[in] ar   array that is to be filtered
 * @param[in] pred predicate function which returns true if the element should
 ...
 */
size_t array_contains_value(Array *ar, void *element,
 ...
 */
size_t array_size(Array *ar) { return ar->size; }
 ...
 */
size_t array_capacity(Array *ar) { return ar->capacity; }
 ...
 * @return the index.
 */
size_t array_iter_index(ArrayIter *iter) { return iter->index - 1; }
 ...
 * of one of the arrays has been reached.
 */
 ...
 * the state of the iterator is invalid, or CC_ERR_VALUE_NOT_FOUND if the
 * element was already removed.
 */
enum cc_stat array_zip_iter_remove(ArrayZipIter *iter, void **out1,
                                   void **out2) {
 ...
 * <code>array_zip_iter_next()</code> without invalidating the iterator.
 *
 * @param[in] iter iterator on which this operation is being performed
 ...
 * CC_ERR_OUT_OF_RANGE.
 */
enum cc_stat array_zip_iter_replace(ArrayZipIter *iter, void *e1, void *e2,
                                    void **out1, void **out2) {
  if ((iter->index - 1) >= iter->ar1->size ||
      (iter->index - 1) >= iter->ar2->size)
    return CC_ERR_OUT_OF_RANGE;
 ...
 * <code>array_zip_iter_next()</code>.
 *
 * @param[in] iter iterator on which this operation is being performed
 *
 * @return current iterator index.
 */
size_t array_zip_iter_index(ArrayZipIter *iter) { return iter->index - 1; }
