 ...
 * @param[in] list1 the consumer list to which the elements are moved
 * @param[in] list2 the producer list from which the elements are moved
 * @param[in] index the index in the first list after which the elements from
 * the second list should be inserted
 *
 * @return CC_OK if the elements were successfully moved, or CC_ERR_OUT_OF_RANGE
 * if the index was not in range.
 */
...
/**
 * Removes and returns the first (head) element of the list and optionally sets
 * the out parameter to the value of the removed element.
 *
 ...
 *
 * @return CC_OK if the element was successfully removed, or
 * CC_ERR_VALUE_NOT_FOUND if the List is already empty.
 */
...
/**
 * Removes and returns the last (tail) element of the list and optionally sets
 * the out parameter to the value of the removed element.
 *
 ...
 *
 * @return CC_OK if the elements were successfully removed, or
 ...
 * CC_ERR_OUT_OF_RANGE if the index was out of range.
 */
enum cc_stat list_replace_at(List *list, void *element, size_t index,
                             void **out) {
  Node *node;
  enum cc_stat status = get_node_at(list, index, &node);
 ...
 * List of length 3, containing [6, 7, 8]. The returned sublist is only a copy
 * of the original lists structure, meaning the data it points to is not copied.
 *
 ...
 * if the specified index range is invalid, or CC_ERR_ALLOC if the memory
 * allocation for the new sublist failed.
 */
...
/**
 * Creates a deep copy of the specified list. This functions copies the
 * structure of the list along with all the data it holds. The element copying
 * is done through the specified copy function that should return a pointer to
 * the copy of the element passed to it.
 *
 * @param[in] list list to be copied
 ...
 *
 * @return CC_OK if the array was successfully created, CC_ERR_INVALID_RANGE if
 * the list is emtpy, or CC_ERR_ALLOC if the memory allocation for the new array
 * failed.
 */
...
/**
 * Returns the number occurrences of the value pointed to by
 * <code>element</code> within the List.
 *
 * @param[in] list list on which the search is performed
 * @param[in] element element being searched for
 ...
 * @return number of matches found.
 */
size_t list_contains_value(List *list, void *element,
 ...
 */
enum cc_stat list_index_of(List *list, void *element,
                           int (*cmp)(const void *, const void *),
                           size_t *index) {
  Node *node = list->head;
  size_t i = 0;
...
static Node *split(List *, Node *b, size_t l,
                   int (*cmp)(void const *e1, void const *e2));
static void merge(Node **, Node **, size_t, size_t,
                  int (*cmp)(void const *e1, void const *e2));
 ...
 *       dereferencing will be required before the data can be used for
 * comparison: e.g. <code>my_type e = *(*((my_type**) ptr));</code>.
 *
 * @param[in] list list to be sorted
 * @param[in] cmp the comparator function that must be of type <code>
 *                int cmp(const void e1*, const void e2*)</code> that
 ...
 *                before the first
 */
void list_sort_in_place(List *list,
                        int (*cmp)(void const *e1, void const *e2)) {
 ...
 * @return
 */
static Node *split(List *list, Node *b, size_t size,
 ...
 * partition starts at the "left" node and ends "l_size" nodes to the right of
 *it. Similarly the right partition starts at the "right" and ends "r_size"
 *nodes to the right. Both "left" and "right" parameters are used for input and
 ...
 * original List that return true on pred(element) without modifying the
 * original list.
 *
 * @param[in] list list that is to be filtered
 * @param[in] pred predicate function which returns true if the element should
 ...
 * @return the index.
 */
size_t list_iter_index(ListIter *iter) { return iter->index - 1; }
 ...
 * @return the index.
 */
size_t list_diter_index(ListIter *iter) { return iter->index - 1; }
 ...
 * <code>list_zip_iter_next()</code> without invalidating the iterator.
 *
 * @note This function should only ever be called after a call to
 ...
 *
 * @return CC_OK if the element was removed successfully, or
 * CC_ERR_VALUE_NOT_FOUND.
 */
 ...
 * CC_ERR_VALUE_NOT_FOUND.
 */
enum cc_stat list_zip_iter_replace(ListZipIter *iter, void *e1, void *e2,
                                   void **out1, void **out2) {
  if (!iter->l1_last || !iter->l2_last)
    return CC_ERR_VALUE_NOT_FOUND;
 ...
 */
size_t list_zip_iter_index(ListZipIter *iter) { return iter->index - 1; }
...
/**
 * Unlinks a node from the list and returns the data that was associated with
 * it.
 *
 ...
 * with the specified element. If no node is associated with the element, NULL
 * is returned instead.
 *
 * @param[in] list the list from which the node is being returned
