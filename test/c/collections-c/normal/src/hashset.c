 ...
 */
void hashset_conf_init(HashSetConf *conf) { hashtable_conf_init(conf); }
 ...
 *
 * @param[in] conf The hashset configuration object. All fields must be
 * initialized.
 ...
 * CC_ERR_VALUE_NOT_FOUND if the value was not found.
 */
 ...
 */
void hashset_remove_all(HashSet *set) { hashtable_remove_all(set->table); }
 ...
 */
size_t hashset_size(HashSet *set) { return hashtable_size(set->table); }
 ...
 */
size_t hashset_capacity(HashSet *set) { return hashtable_capacity(set->table); }
