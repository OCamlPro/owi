 ...
 */
void stack_conf_init(StackConf *conf) { array_conf_init(conf); }
 ...
 * struct. The allocation may fail if the underlying allocator fails. It may
 * also fail if the values of exp_factor and capacity in the StackConf do not
 * meet the following condition: <code>exp_factor < (CC_MAX_ELEMENTS /
 * capacity)</code>.
 *
 ...
 */
size_t stack_size(Stack *stack) { return array_size(stack->v); }
 ...
 */
void stack_map(Stack *stack, void (*fn)(void *)) { array_map(stack->v, fn); }
 ...
 * CC_ERR_OUT_OF_RANGE.
 */
enum cc_stat stack_zip_iter_replace(StackZipIter *iter, void *e1, void *e2,
                                    void **out1, void **out2) {
