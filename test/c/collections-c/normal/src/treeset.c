 ...
 */
void treeset_conf_init(TreeSetConf *conf) { treetable_conf_init(conf); }
 ...
 */
enum cc_stat treeset_new(int (*cmp)(const void *, const void *),
                         TreeSet **set) {
  TreeSetConf conf;
 ...
 *
 * @param[in] conf  TreeSet configuration struct. All fields must be
 * initialized.
 ...
 */
void treeset_remove_all(TreeSet *set) { treetable_remove_all(set->t); }
 ...
 */
size_t treeset_size(TreeSet *set) { return treetable_size(set->t); }
