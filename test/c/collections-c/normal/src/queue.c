 ...
 */
void queue_conf_init(QueueConf *conf) { deque_conf_init(conf); }
 ...
 *
 * The Queue is allocated using the allocators specified in the QueueConf
 ...
 */
size_t queue_size(Queue const *const queue) { return deque_size(queue->d); }

/**
 * Applies the function fn to each element of the Queue.
 *
 * @param[in] queue the queue on which this operation is performed
 * @param[in] fn the operation function that is to be invoked on each queue
 * element
 */
 ...
 * CC_ERR_OUT_OF_RANGE.
 */
enum cc_stat queue_iter_replace(QueueIter *iter, void *replacement,
                                void **out) {
 ...
 * CC_ERR_OUT_OF_RANGE.
 */
enum cc_stat queue_zip_iter_replace(QueueZipIter *iter, void *e1, void *e2,
                                    void **out1, void **out2) {
