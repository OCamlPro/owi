...
static void copy_buffer(Deque const *const deque, void **buff,
                        void *(*cp)(void *));
 ...
 *
 * The Deque is allocated using the allocators specified in the DequeConf
 ...
 */
enum cc_stat deque_replace_at(Deque *deque, void *element, size_t index,
                              void **out) {
  if (index >= deque->size)
    return CC_ERR_OUT_OF_RANGE;
 ...
 * memory allocation for the copy failed.
 */
enum cc_stat deque_copy_deep(Deque const *const deque, void *(*cp)(void *),
                             Deque **out) {
  Deque *copy = deque->mem_alloc(sizeof(Deque));
 ...
 * @return the number of occurrences of the element
 */
size_t deque_contains_value(Deque const *const deque, const void *element,
 ...
 */
enum cc_stat deque_index_of(Deque const *const deque, const void *element,
                            size_t *index) {
  size_t i;
 ...
 */
size_t deque_size(Deque const *const deque) { return deque->size; }
 ...
 */
size_t deque_capacity(Deque const *const deque) { return deque->capacity; }
 ...
 * original Deque that return true on pred(element) without modifying the
 * original deque.
 *
 * @param[in] deque deque that is to be filtered
 ...
 * new deque failed.
 */
enum cc_stat deque_filter(Deque *deque, bool (*pred)(const void *),
                          Deque **out) {
  if (deque_size(deque) == 0)
    return CC_ERR_OUT_OF_RANGE;
 ...
 * only copies the elements instead of the whole buffer and also realigns the
 ...
 */
enum cc_stat deque_iter_replace(DequeIter *iter, void *replacement,
                                void **out) {
 ...
 * @return the index
 */
size_t deque_iter_index(DequeIter *iter) { return iter->index - 1; }
 ...
 * <code>deque_zip_iter_next()</code> without invalidating the iterator.
 *
...
enum cc_stat deque_zip_iter_add(DequeZipIter *iter, void *e1, void *e2) {
  ...
  /* While this check is performed by a call to deque_add_at, it is necessary to
     know in advance whether both deque buffers have enough room before
     inserting new elements because this operation must insert either both
     elements, or none.*/
 ...
 * was already removed.
 */
enum cc_stat deque_zip_iter_remove(DequeZipIter *iter, void **out1,
                                   void **out2) {
  if (iter->last_removed)
    return CC_ERR_VALUE_NOT_FOUND;
 ...
 * CC_ERR_OUT_OF_RANGE.
 */
enum cc_stat deque_zip_iter_replace(DequeZipIter *iter, void *e1, void *e2,
                                    void **out1, void **out2) {
  if ((iter->index - 1) >= iter->d1->size ||
      (iter->index - 1) >= iter->d2->size)
    return CC_ERR_OUT_OF_RANGE;
 ...
 * <code>deque_zip_iter_next()</code>.
 *
 * @param[in] iter Iterator on which this operation is being performed
 *
 * @return current iterator index
 */
size_t deque_zip_iter_index(DequeZipIter *iter) { return iter->index - 1; }
