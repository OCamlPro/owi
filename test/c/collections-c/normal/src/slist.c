...
static enum cc_stat get_node_at(SList *list, size_t index, SNode **node,
                                SNode **prev);
static enum cc_stat get_node(SList *list, void *element, SNode **node,
                             SNode **prev);
 ...
 */
static INLINE void splice_between(SList *l1, SList *l2, SNode *base,
                                  SNode *end) {
  if (!base) {
 ...
 *
 * @return CC_OK if the element was successfully removed, or
 * CC_ERR_VALUE_NOT_FOUND if the list is empty.
 */
...
/**
 * Replaces an element at the specified location and optionally sets the out
 ...
 */
enum cc_stat slist_replace_at(SList *list, void *element, size_t index,
                              void **out) {
  SNode *prev = NULL;
  SNode *node = NULL;
...
/**
 * Returns the number of occurrences of the value pointed to by
 * <code>element</code> within the specified SList.
 *
 * @param[in] list SList on which the search is performed
 * @param[in] element element being searched for
 ...
 */
size_t slist_contains_value(SList *list, void *element,
 ...
 * array is the same as the size of the list from which the array was
 * constructed.
 *
 ...
 */
enum cc_stat slist_sort(SList *list,
 ...
 * original SList that return true on pred(element) without modifying the
 * original SList.
 *
 ...
 * new SList failed.
 */
enum cc_stat slist_filter(SList *list, bool (*pred)(const void *),
                          SList **out) {
 ...
 * @return the index
 */
size_t slist_iter_index(SListIter *iter) { return iter->index - 1; }
 ...
 * <code>slist_zip_iter_next()</code> without invalidating the iterator.
 *
 * @note This function should only ever be called after a call to <code>
 ...
 *
 * @return CC_OK of the element was successfully removed, or
 * CC_ERR_VALUE_NOT_FOUND.
 */
enum cc_stat slist_zip_iter_remove(SListZipIter *iter, void **out1,
                                   void **out2) {
 ...
 *
 * @return CC_OK of the element was successfully replaced, or
 * CC_ERR_VALUE_NOT_FOUND.
 */
enum cc_stat slist_zip_iter_replace(SListZipIter *iter, void *e1, void *e2,
                                    void **out1, void **out2) {
  if (!iter->l1_current || !iter->l2_current)
    return CC_ERR_VALUE_NOT_FOUND;
 ...
 * <code>slist_zip_iter_next()</code>.
 *
 * @param[in] iter Iterator on which this operation is being performed
 *
 * @return current iterator index
 */
size_t slist_zip_iter_index(SListZipIter *iter) { return iter->index - 1; }

/**
 * Unlinks the node from the list and returns the data that was associated with
 * it and also adjusts the head / tail of the list if necessary.
 *
 * @param[in] list the list from which the node is being unlinked
 * @parma[in] node the node being unlinked
 * @param[in] prev the node that immediately precedes the node that is being
 * unlinked
 *
