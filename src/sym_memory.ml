...
module Memory = struct
  ...
  let concretize_addr (addr : Expression.t) : address =
    match addr with Val (Num (I32 a)) -> a | _ -> assert false
  ...
  let load_8_u (mem : t) (a : Expression.t) : Expression.t = load_8_s mem a
  ...
  let load_16_s (mem : t) (a : Expression.t) : Expression.t =
    let res =
      ...
      Extract (Val (Num (I64 0L)), 3, 0) ++ b1 ++ b2
    in
    Option.get res

  let load_16_u (mem : t) (a : Expression.t) : Expression.t = load_16_s mem a

  let load_32 (_mem : t) (_a : Expression.t) : Expression.t = assert false

  let load_64 (_mem : t) (_a : Expression.t) : Expression.t = assert false
