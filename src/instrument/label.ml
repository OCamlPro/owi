          ...
          let body =
            Annotated.map
              (fun expr ->
                incr count;
                Annotated.dummies
                  [ Binary.Simple (I32 (Const (Int32.of_int !count)))
                  ...
                  | Binary.Br_if idx ->
                    ...
                    Binary.If_else
                      ...
                      , Annotated.dummy_deep
                          [ Binary.Simple (I32 (Const (Int32.of_int count_true)))
                          ; Call cov_label_set_idx
                          ; Br (idx + 1)
                          ]
                      , Annotated.dummy_deep
                          [ Binary.Simple
                              (I32 (Const (Int32.of_int count_false)))
                          ; Call cov_label_set_idx
                          ] )
                  | Binary.If_else (id, typ, true_branch, false_branch) ->
                    incr count;
                    ...
                    let true_branch =
                      Annotated.map
                        (fun expr ->
                          Annotated.dummies
                            [ Binary.Simple
                                (I32 (Const (Int32.of_int count_true)))
                            ; Call cov_label_set_idx
                            ]
                          @ expr )
                        true_branch
