...
let set_max_depth stats path_depth =
  let rec loop () =
    let max_depth = Atomic.get stats.max_depth in
    if max_depth >= path_depth then ()
    else begin
      let success =
        Atomic.compare_and_set stats.max_depth max_depth path_depth
      in
      if success then () else loop ()
    end
  in
  loop ()
