...
let replace tbl = Symbolic_choice.map_state (Thread.replace_table tbl)
...
let of_concrete ~modul ~id (original : Concrete_table.t) =
  let _i, data =
    Array.fold_left
      (fun (i, map) v ->
  ...
  in
  { data; limits = original.limits; typ = original.typ; modul; id }
