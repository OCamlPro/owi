...
let[@inline] fork ~parent ~child : 'a t =
  let* fork = lift_schedulable @@ Scheduler.Schedulable.fork parent child in
  fork
...
let select_inner ~with_breadcrumbs (cond : Symbolic_boolean.t)
  ...
  | _ ->
    ...
    fork ~parent:(fork ~parent:stop ~child:false_branch) ~child:true_branch
[@@inline]
