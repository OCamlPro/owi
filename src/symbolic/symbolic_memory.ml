...
let replace m k v =
  let data = Map.add k v m.data in
  { m with data }
...
let storen m a v n =
  ...
  let rec loop i m =
    if i >= n then m
      ...
      let m = replace m a' v' in
      loop (i + 1) m
  in
  let memory = loop 0 m in
  let+ thread in
  let memories = Thread.memories thread in
  Symbolic_memory_collection.set_memory memories memory
...
let blit_string m str ~src ~dst ~len =
  ...
  let rec loop i =
    if i >= len then return ()
      ...
      let a = address_i32 (Int32.of_int (dst + i)) in
      let* () =
        storen m a (Smtml.Expr.value (Bitv (Smtml.Bitvector.of_int8 byte))) 1
      in
      loop (i + 1)
  in
  loop 0
