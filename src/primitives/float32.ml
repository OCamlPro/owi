...
let is_inf x = Int32.eq x pos_inf || Int32.eq x neg_inf

let is_nan x =
  let xf = Int32.float_of_bits x in
  Float.is_nan xf

let is_pos_nan f = Int32.eq f pos_nan

let is_neg_nan f = Int32.eq f neg_nan
...
let nearest x =
  ...
  if Float.equal xf 0.0 then x
    ...
    let delta_u_d = Float.compare um dm in
    let u_or_d =
      delta_u_d < 0
      || delta_u_d = 0
         &&
         let h = u /. 2. in
         Float.equal (Float.floor h) h
    in
