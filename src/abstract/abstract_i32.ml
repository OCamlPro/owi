...
let of_int32 ctx i =
  Abstract_domain.Binary_Forward.biconst ~size (Z.of_int32_unsigned i) ctx
...
let shl ctx x1 x2 =
  (* We use a mask to ensure x2 is within [0, 31] as we should be doing modulo 32 of the size as per official specs. *)
  (* TODO : test how expensive they are globally, add a check before doing them if too expensive *)
  let mask = of_int32 ctx 31l in
...
let wrap_i64 ctx x =
  Abstract_domain.Binary_Forward.bextract ~index:Units.In_bits.zero
    ~size:Units.In_bits.s32 ~oldsize:(Units.In_bits.of_int 64) ctx x
